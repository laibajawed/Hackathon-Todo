...
async def chat(
    ...
    # Check conversation length limit (prevent performance degradation)
    MAX_MESSAGES_PER_CONVERSATION = 100  # Configurable limit
    if conversation.message_count >= MAX_MESSAGES_PER_CONVERSATION:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Conversation has reached maximum length ({MAX_MESSAGES_PER_CONVERSATION} messages). Please start a new conversation."
        )
    ...
    try:
        import logging
        from agent.runner import run_agent_with_mcp
        from config import settings
        from utils.cache import response_cache, is_cacheable_query

        logging.info(f"Starting agent execution for user {current_user['user_id']}")

        # Check cache for read-only queries
        cached_response = None
        if is_cacheable_query(request.message):
            cached_response = response_cache.get(
                user_id=str(current_user["user_id"]),
                message=request.message
            )
            if cached_response:
                logging.info(f"Cache hit for user {current_user['user_id']}")

        if cached_response:
            # Use cached response
            agent_result = cached_response
        else:
            # Run agent with MCP integration (handles subprocess spawning and tool retrieval)
            ...
            logging.info(f"Agent execution completed: {agent_result.get('response', '')[:100]}")

            # Cache the response if it's a cacheable query
            if is_cacheable_query(request.message) and not agent_result.get("error", False):
                response_cache.set(
                    user_id=str(current_user["user_id"]),
                    message=request.message,
        ...
        assistant_response = f"I'm having trouble processing your request: {str(e)}"
        tokens_used = count_tokens(assistant_response)
        error_occurred = True
